...
//        $this->serverCache->setLogLevel($this->getLogLevel());
    }
    ...
    public function documentcheckout(array $data): ?array
    {
        ...
        $separate = Entity::get('ORDER_SEPARATE_BY_SELLERS');
        $paymentForNew = Entity::get('ORDER_PAYMENT_FOR_NEW');

        // Продавець за замовчуванням маркетплейса
        $seller = $this->marketplace()['sellers'][0]['uid'];

        // Перерахунок суми
        // не враховуємо товари що мали статус "Немає в наявності"
        ...
        foreach ($_specs as $k => $spec) {
            ...
            // Отримувач оплати за замовчуванням маркетплейса
            $payee = $this->marketplace()['payee'][0]['uid'];

            if ($separate) {
                // Отримувач оплати вказаний у Продавця
                $payee = $this->sellers($k)['payee'] ? $this->sellers($k)['payee'][0]['uid'] : $payee;
            }
            ...
            if ($paymentForNew && $_doc['Document']['doc_sum'] > 0) {
                $signs = [
                    'doc_sign_2' => "true",
...
//                            ];
    ...
    /**
     * Підготовка даних для Трекінг та Оплата Замовлення
     * @param string $uid
     * @return array
     */
    public function documenttracking(string $uid): array
    {
        ...
        $_payee = $this->payee($_doc['Document']['doc_payee']);

        if (isset($_payee['pt']) && is_array($_payee['pt'])) {
            $_payee['pt'] = array_filter($_payee['pt'], function($item) {
                return !isset($item['enabled']) || $item['enabled'] !== false;
            });
        }

        $_doc['Document']['payee'] = $_payee;
        return $_doc;
    }
    ...
    public function payee(string $uid = null): array
    {
        ...
        return $uid ? recursive_array_search_key_value($pe, $uid, 'uid', true) : array_search_by_key($pe,0);
    }
    ...
    public function documentList(array $params = null): array
    {
        ...
        $paging = [
            ...
            'from' => 0,
            'to' => 0,
            'length' => !empty($params['length']) ? (int)$params['length'] : $this->_paging_length
        ];
        ...
        foreach ($items as $item) {
            ...
            if (isset($params['search']) && strlen($params['search']) > 3) {
                if (
                    ...
                    stripos($_doc['Document']['session'], $params["search"]) !== false
                ) {
            ...
            if ($bAddThisItem) {
                ...
                $docs[strtotime(substr($_doc['Document']['doc_date_create'], 0, 19)) . '-' . $_doc['Document']['doc_uid']] = $_doc['Document'];
            }
            $n++;
        }
