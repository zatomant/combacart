...
use Comba\Bundle\Standalone\Server;
use Comba\Core\Entity;
use Comba\Core\Logs;
use function Comba\Functions\array_search_by_key;

if (!defined('AUTHPASS')) {
    header('Location: //' . Entity::get('SERVER_HOST'));
    exit;
}
...
$lg = new Logs('api');
$lg->setGlobalContext(['ip' => (new RemoteIP)->get_ip_address()]);
...
if (!empty($data)) {
    ...
    if ('CabinetRead' == $callme) {
        if ($data['methodProperties']['Marketplace'] && $data['methodProperties']['User']['session']) {
            $res = (new Server())->documentList(
                [
                    'search' => array_search_by_key($data['methodProperties']['User'], 'session'),
                    'page' => array_search_by_key($data['methodProperties'],'page'),
                    'length' => Entity::get('PAGE_CABINET_LENGTH')
                ]
            );
            $ret = !empty($res) ? '{"result":"ok","Document":' . json_encode(array_search_by_key($res, 'docs')) . ', "paging":' . json_encode(array_search_by_key($res, 'paging')) . '}' : '{"result":"error"}';
        }
    }
    ...
    if ('DeliveryList' == $callme) {
        if ($data['methodProperties']['Seller']) {
            ...
            foreach ($res as $k => $el) {
                if (isset($el['enabled']) && $el['enabled'] == false) {
                    unset($res[$k]);
                }
            }
            $ret = !empty($res) ? '{"result":"ok","Document":' . json_encode($res) . '}' : '{"result":"error"}';
