...
class ModxOper extends Oper
{
    ...
    public function getLanguageList(): ?array
    {
        ...
        foreach ($files as $file) {
            ...
            if (COMBAMODX_LANGUAGE == $path_parts['filename']) {
                $langs[] = ['url' => $this->getModx()->getConfig('site_url') . COMBAMODX_PAGE_COMBA, 'label' => $_lang['_language']];
            } else {
                $langs[] = ['url' => $this->getModx()->getConfig('site_url') . $path_parts['filename'] . '/' . COMBAMODX_PAGE_COMBA, 'label' => $_lang['_language']];
            }
        }
