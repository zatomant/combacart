...
class ModxUser extends ModxOptions
{
    ...
    public function __construct($modx)
    {
        ...
        $this->setOptions('GetSessionName', COMBAMODX_SESSION_NAME . strtolower(get_class($this)));
        $this->prepareUserEnv();
    }
    ...
    public function prepareUserEnv(array $user = null)
    {
        ...
        if ($this->isBot()) {
            ...
            if (is_object($this->getModx())) {
                ...
                if (!empty($user_id)) $user = $this->getModx()->getWebUserInfo($user_id);
    ...
    {
        if ($modxuserid !== null) {
            $site = preg_replace("/[^a-zA-Z0-9]+/", "", filter_var(COMBAMODX_SERVER_NAME, FILTER_SANITIZE_URL));
