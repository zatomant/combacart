...
class Manager extends ModxOptions
{
    ...
    public function render(): ?string
    {
        ...
        if (!empty($userid)) {
            ...
            $action = ($user['usertype'] == 'manager') ? 'orderlist' : 'restricted';
        }
    ...
    private function renderAction(): string
    {
        ...
        $actionsMap = [
            COMBAMODX_PAGE_COMBA => 'readOrderList',
        ...
        preg_match('#(?:/([^/]+))?/' . COMBAMODX_PAGE_COMBA . '/([^/]+)/([^/]+)(?:/([^/]+))?#', $uri, $matches);
        $action = $matches[2] ?? '';
        $uid = $matches[3] ?? '';
        $args = $matches[4] ?? '';

        if (empty($action) && empty($uid)) {
            $action = COMBAMODX_PAGE_COMBA;
        }
    ...
    public function saveOrder(array $params): string
    {
        ...
        (new Server())->documentupdate($uid, $data);
    ...
    public function readOrderList($params): string
    {
        ...
        if (empty($params) || empty($params[0])) {
            $tpl->addGlobal('App',
                [
                    'name' => COMBAMODX_NAME,
                    'version' => COMBAMODX_VERSION,
                    'fver' => COMBAMODX_FILE_VER
                ]
            );
